...
public class ErrandEditModel : PageModel
{
    ...
    public ErrandEditModel(IElevatorService elevatorService, IErrandService errandService,
        IErrandCommentService errandCommentService, ITechnicianService technicianService)
    {
    ...
    [BindProperty] public ErrandModel Errand { get; set; }
    ...
    public async Task<IActionResult> OnGetAsync(string elevatorId, string errandId)
    {
        ...
        SelectTechnicianEdit = await _technicianService.SelectListTechniciansEditAsync(Errand.Technician.Id.ToString());
    ...
    {
        Errand.Technician = await _technicianService.GetTechnicianByIdAsync(Errand.Technician.Id.ToString());
        Errand.Comments = await _errandCommentService.GetErrandCommentsFromErrandIdAsync(Errand.Id.ToString());

        if (ModelState.IsValid)
        {
            await _errandService.EditErrandAsync(Errand.Id.ToString(), Errand, Errand.Technician.Id.ToString(),
                Errand.Comments);
