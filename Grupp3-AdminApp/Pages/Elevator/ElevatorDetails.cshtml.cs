...
public class ElevatorDetailsModel : PageModel
{
    ...
    public ElevatorDetailsModel(IConfiguration configuration, IElevatorService elevatorService,
        IErrandService errandService)
    {
    ...
    public async Task<IActionResult> OnPostChangeLevel(string elevatorId, int targetLevel)
    {
        try
        {
            using var serviceClient =
                ServiceClient.CreateFromConnectionString(_configuration.GetValue<string>("IoTHubConnection"));
        ...
        return RedirectToPage("ElevatorDetails", new {elevatorId});
    }
